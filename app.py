...
if st.button("🚀 توليد البطاقات الستة الهجومية"):
    if len(d1) != 50 or len(d2) != 50:
        st.error(f"⚠️ خطأ: يجب أن تحتوي كل سحبة على 50 رقماً بالضبط! السحبة الأولى فيها {len(d1)} والثانية {len(d2)}.")
    else:
        st.success("✅ البيانات مكتملة، جاري المعالجة...")
